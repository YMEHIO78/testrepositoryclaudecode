...
The AI assistant. See the Assistant section of the README for the safety
model; this covers the storage.

**`agent_messages.content` is `JSONB` holding the raw Claude content block
array**, not extracted text. Thinking blocks and `tool_use` blocks have to
go back to the API unmodified on the next turn — storing only the text
would break multi-turn tool use in a way that looks like the model losing
its place. `input_tokens` / `output_tokens` are stored per message so the
UI can show what the feature has cost.

**`agent_actions` is the approval queue, and it is the whole safety model.**
A write tool does not touch the database: it inserts a row here with
`status = 'pending'` and the turn ends. `decideAction` is the only code
path that executes one.

| Column | Notes |
|---|---|
| `kind` | the write tool's name; maps to an entry in `WRITE_TOOLS` |
| `input` | `JSONB`, the arguments the model supplied |
| `summary` | the one line a human reads when deciding — generated by that tool's `summarise` |
| `status` | `pending` → `approved` → `executed`, or `rejected`, or `failed` |

**Approval is idempotent by construction.** The status flips inside the
same `UPDATE ... WHERE status = 'pending'` that claims the row, so a
second concurrent approve matches nothing and returns null rather than

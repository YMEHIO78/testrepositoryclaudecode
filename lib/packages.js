...
// A short "3 x Manual Data · 1 x Data Analysis" per client, for the
// pipeline board's cards. Built alongside the values in one pass rather
// than a query per card.
async function summariesByClient() {
  const { rows } = await pool.query(
    `SELECT cp.client_id, p.name, cp.quantity
       FROM client_packages cp
       JOIN packages p ON p.id = cp.package_id
      WHERE cp.quantity > 0
      ORDER BY p.sort_order, lower(p.name)`,
  ...
  const map = new Map();
  for (const r of rows) {
    const line = `${r.quantity} × ${r.name}`;
    map.set(r.client_id, map.has(r.client_id) ? `${map.get(r.client_id)} · ${line}` : line);
  }
  return map;
}
...
module.exports = {
  ...
  clientPackages, setQuantity, valueFor, valuesByClient, summariesByClient,
};

...
async function createBooking(eventType, { startsAt, name, email, notes }) {
  ...
  try {
    ...
    // Attribute the meeting to a client if the booker's address belongs to
    // one of their contacts. After the commit deliberately: a booking that
    // succeeded must not be rolled back because the lookup failed.
    calendar.linkByAttendeeEmail(eventRows[0].id, email)
      .catch((err) => console.error('Could not link booking to a client:', err.message));

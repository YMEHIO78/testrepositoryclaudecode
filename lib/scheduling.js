  ...
  while (day <= lastDay && slots.length < 500) {
    ...
    for (const w of windows) {
      ...
      while (cursor.plus({ minutes: duration }) <= windowClose) {
        ...
        if (withinRange) {
          // Buffers extend the footprint that must be clear, without
          // moving the meeting itself.
          const guardStart = slotStart.minus({ minutes: bufferBefore }).toMillis();
          const guardEnd = slotEnd.plus({ minutes: bufferAfter }).toMillis();
          const clash = busy.some((b) => overlaps(guardStart, guardEnd, b.start, b.end));
  ...
  try {
    ...
    const { rows: conflicts } = await client.query(
      `SELECT 1 FROM calendar_events
        WHERE starts_at < $2
          AND COALESCE(ends_at, starts_at + interval '1 hour') > $1
        LIMIT 1`,
      [guardStart.toISO(), guardEnd.toISO()],
    );
    if (conflicts.length) {
      await client.query('ROLLBACK');

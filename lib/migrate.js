...
async function migrate() {
  ...
  // the app to Dropbox must not orphan everything already in the bucket,
  // and a file that predates the switch keeps working because its own
  // row still says where it lives.
  //
  // storage_key means the S3 object key for 'bucket' and the file id for
  // 'dropbox'. Both are opaque handles, so one column carries either.
  await pool.query(`

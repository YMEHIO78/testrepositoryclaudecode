...
async function migrate() {
  ...
  // Which store a file's bytes are in. Per file, not per app: switching
  // the app to OneDrive must not orphan everything already in the
  // bucket, and a file that predates the switch keeps working because
  // its own row still says where it lives.
  //
  // storage_key means the S3 object key for 'bucket' and the Graph item
  // id for 'onedrive'. Both are opaque handles, so one column carries
  // either.
  await pool.query(`
    ALTER TABLE files ADD COLUMN IF NOT EXISTS storage_provider TEXT NOT NULL DEFAULT 'bucket';
  `);

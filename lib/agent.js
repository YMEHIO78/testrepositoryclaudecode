...
const READ_TOOLS = {
  ...
  // Deliberately does NOT include client_systems or client_secrets. A
  // client's stored credentials must never be read into a model prompt —
  // not the values, not the labels, not the usernames. If a future read
  // tool needs "everything about a client", it still stops short of the
  // vault. See lib/vault.js.
  get_client: {

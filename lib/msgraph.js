...
// Reads the (unencrypted, non-secret) claims out of an access token so a
// broken connection can be diagnosed without exposing the token itself.
// Not a signature check — Graph validates the token, we just inspect it.
function describeToken(accessToken) {
  if (typeof accessToken !== 'string' || !accessToken) return { present: false };
  const parts = accessToken.split('.');
  if (parts.length !== 3) {
    return { present: true, isJwt: false, length: accessToken.length };
  }
  try {
    ...
    return {
      ...
      audience: claims.aud,
      scopes: claims.scp,
      signedInAs: claims.upn || claims.preferred_username,
      tenant: claims.tid,
      expiresAt: claims.exp ? new Date(claims.exp * 1000).toISOString() : null,
      expired: claims.exp ? claims.exp * 1000 < Date.now() : null,
    };
  } catch (err) {
    return { present: true, isJwt: true, claimsUnreadable: true };
  }
}

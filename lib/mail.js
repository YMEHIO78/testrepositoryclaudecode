...
// Used to build the exact bytes of an outgoing message so the same ones
// can be filed in Sent. Loaded defensively: it is an internal path in
...
try {
  ...
  console.error('mail-composer unavailable; sent mail will not be copied to Sent.');
}
...
// Puts a copy of what was just sent into the mailbox's Sent folder.
//
// SMTP delivers the message and then forgets it — nothing about sending
// puts a copy anywhere you can see. Without this, mail sent from here
// exists only in the recipient's inbox: not in Sent on the server, not
// in webmail, not on the phone, and not in this app either. "Did that
// actually go?" with no way to check is a bad position to be in about
// client correspondence.
async function saveToSent(email, raw) {
  if (!raw) return false;
  try {
    return await withInbox(email, async (client) => {
      const sent = await specialUseFolder(client, '\\Sent');
      if (!sent) return false;
      // \Seen because you wrote it — an unread count that climbs as you
      // send is just noise.
      await client.append(sent, raw, ['\\Seen']);
      return true;
    });
  } catch (err) {
    // The message is already delivered by this point. Failing to file a
    // copy must not be reported as a failure to send.
    console.error(`Could not save a copy to Sent for ${email}:`, err.message);
    return false;
  }
}

async function sendMail({ from, to, subject, text, replyTo, inReplyTo, references }) {
  ...
  // Build the bytes once so the copy filed in Sent is the same message
  // that went out, rather than a re-rendering of it that could differ.
  let raw = null;
  if (MailComposer) {
    try {
      raw = await new MailComposer(message).compile().build();
    } catch (err) {
      console.error('Could not compose a raw message:', err.message);
    }
  }
  ...
  if (raw) {
    ...
    await transporterFor(account).sendMail(message);
  }

  const saved = await saveToSent(account.email, raw);
  return { from: account.email, savedToSent: saved };
}
...
// Goes through sendMail so a reply is filed in Sent like anything else.
// It used to send directly, which meant replies left no trace anywhere.

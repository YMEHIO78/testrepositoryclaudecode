...
async function listEvents({ from, to, clientId } = {}) {
  const clauses = [];
  const params = [];
  if (from) { params.push(from); clauses.push(`e.starts_at >= $${params.length}`); }
  if (to) { params.push(to); clauses.push(`e.starts_at < $${params.length}`); }
  if (clientId) { params.push(clientId); clauses.push(`e.client_id = $${params.length}`); }
  const where = clauses.length ? `WHERE ${clauses.join(' AND ')}` : '';

  const { rows } = await pool.query(
    `SELECT e.id, e.title, e.starts_at, e.ends_at, e.all_day, e.location, e.notes,
            e.source, e.google_event_id, e.client_id, c.name AS client_name
       FROM calendar_events e
       LEFT JOIN clients c ON c.id = e.client_id
       ${where} ORDER BY e.starts_at`,
    params,
  );
...
async function setGoogleEventId(id, googleEventId) {
  await pool.query(`UPDATE calendar_events SET google_event_id = $2 WHERE id = $1`, [id, googleEventId]);
}

async function createEvent({ title, startsAt, endsAt, allDay, location, notes, clientId }) {
  const { rows } = await pool.query(
    `INSERT INTO calendar_events (title, starts_at, ends_at, all_day, location, notes, client_id)
     VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
    [title, startsAt, endsAt || null, !!allDay, location || null, notes || null, clientId || null],
  );
  return toEvent(rows[0]);
}

// Links a booking to a client by its attendee's email address, using the
// same contact matching the inbox uses to label mail. Returns the client
// id so the caller can log what happened.
//
// Best-effort by design: an unmatched booking is simply an untagged one,
// which is what an enquiry from a stranger actually is. Guessing at a
// client from a name would put meetings on the wrong record.
async function linkByAttendeeEmail(eventId, email) {
  if (!email) return null;
  const crm = require('./crm');
  const matches = await crm.matchEmails([email]);
  const hit = matches[String(email).toLowerCase().trim()];
  if (!hit?.clientId) return null;

  await pool.query(
    `UPDATE calendar_events SET client_id = $2 WHERE id = $1 AND client_id IS NULL`,
    [eventId, hit.clientId],
  );
  return hit.clientId;
}
...
async function updateEvent(id, { title, startsAt, endsAt, allDay, location, notes, clientId }) {
  const { rows } = await pool.query(
    `UPDATE calendar_events SET
       ...
       location = $6,
       notes = $7,
       -- undefined leaves it, '' clears it, an id sets it. Same shape as
       -- the client page's inline dropdowns.
       client_id = CASE WHEN $8::text IS NULL THEN client_id
                        WHEN $8 = '' THEN NULL ELSE $8::int END,
       updated_at = now()
     WHERE id = $1 RETURNING *`,
    [id, title ?? null, startsAt ?? null, endsAt || null, allDay ?? null, location || null,
      notes || null, clientId === undefined ? null : String(clientId ?? '')],
  );
  return rows.length ? toEvent(rows[0]) : null;
}

// Returns the deleted row so the caller can clean up its Google mirror.
async function deleteEvent(id) {
  const { rows } = await pool.query(`DELETE FROM calendar_events WHERE id = $1 RETURNING *`, [id]);
  return rows.length ? toEvent(rows[0]) : null;
}

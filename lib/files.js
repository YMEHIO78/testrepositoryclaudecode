...
async function listFiles({ clientId, projectId } = {}) {
  const where = [];
  const params = [];
...
async function upload({ name, buffer, contentType, clientId, projectId, notes }) {
  ...
  const { rows } = await pool.query(
    `INSERT INTO files (name, storage_key, content_type, size_bytes, client_id, project_id, notes)
     VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
    [name, key, contentType || null, buffer.length, clientId || null, projectId || null, notes || null],
  );

...
function isConfigured() {
  return bucketConfigured() || onedrive.isConfigured();
}
...
// OneDrive shows real filenames to whoever opens the drive, so unlike an
// S3 key the name is worth keeping readable. Still stripped of anything
// that could act as a path or upset Graph.
...
// Where new files go. OneDrive when it is connected, the bucket
// otherwise — so losing the OneDrive connection degrades to the old
...
async function activeProvider() {
  try {
    if (onedrive.isConfigured() && await onedrive.isConnected()) return 'onedrive';
  } catch (err) {
    console.error('Could not check OneDrive, falling back to the bucket:', err.message);
  }
  return 'bucket';
}
...
async function upload({ name, buffer, contentType, clientId, projectId, folderId, notes }) {
  ...
  if (provider === 'onedrive') {
    ...
    const item = await onedrive.upload(`${key}-${safeName(name)}`, buffer, contentType);
    storageKey = item.id;
  } else {
...
// A file uploaded to the bucket before OneDrive was connected must keep
// downloading from the bucket for ever.
async function download(id) {
  const file = await getFile(id);
  if (!file) return null;

  if (file.storageProvider === 'onedrive') {
    return { file, buffer: await onedrive.download(file.storageKey) };
  }
...
async function replaceContent(id, buffer, contentType) {
  ...
  if (file.storageProvider === 'onedrive') {
    // PUT to the item's own content endpoint overwrites in place and
    // keeps the id — and OneDrive keeps the previous copy in its version
    // history, which the bucket cannot do at all.
    await onedrive.graph(`/me/drive/items/${encodeURIComponent(file.storageKey)}/content`, {
      method: 'PUT', raw: buffer, contentType: contentType || file.contentType,
    });
  } else {
    await s3().send(new PutObjectCommand({
      Bucket: process.env.FILES_BUCKET,
...
async function remove(id) {
  ...
  try {
    if (file.storageProvider === 'onedrive') {
      await onedrive.remove(file.storageKey);
    } else {
...
module.exports = {
  MAX_BYTES, isConfigured, bucketConfigured, activeProvider, check,

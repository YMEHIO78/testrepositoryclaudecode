...
//
// This is as far as Wave's public API goes on the expense side, and the
// limit was confirmed by introspecting the live schema rather than
// assumed. `Business` has no transactions field, the root `Query` has no
// transaction entry point, and the `Transaction` type exposes only `id`.
// The only money-transaction operations in the whole schema are the
// `moneyTransactionCreate` mutations — the API can write an expense and
// cannot read one back.
//
// So there is no per-expense feed to build: no dates, vendors or line
// items. What is readable is the chart of accounts, which gives the total
// booked against each expense category. That is a real ledger figure, and
// it is what a P&L expense section shows.
const EXPENSES_QUERY = `
  query Expenses($businessId: ID!, $page: Int!, $pageSize: Int!) {
    business(id: $businessId) {
      id
      currency { code }
      accounts(page: $page, pageSize: $pageSize, types: [EXPENSE]) {
        pageInfo { totalCount }
        edges {
          node {
            id
...
async function fetchExpenses({ pageSize = 100 } = {}) {
  const creds = await loadCredentials();
  if (!creds) return null;
  if (!creds.businessId) throw new Error('No Wave business selected.');

  const data = await gql(creds.token, EXPENSES_QUERY, {
    businessId: creds.businessId,
    page: 1,
    pageSize,
  });

  const conn = data?.business?.accounts;
  const accounts = (conn?.edges || [])
    .map((e) => e.node)
    .filter((n) => !n.isArchived)
    // `balance` is a Decimal in MAJOR units — dollars — unlike invoices,
    // where Money.minorUnitValue is already cents. Reading it as cents
    // would be wrong by a factor of a hundred. This app is integer cents
    // everywhere, so convert once, here.
    .map((n) => ({
      id: n.id,
      name: n.name,
      description: n.description || null,
      subtype: n.subtype?.name || null,
      balanceCents: Math.round(Number(n.balanceInBusinessCurrency ?? n.balance ?? 0) * 100),
    }))
    .filter((a) => a.balanceCents !== 0)
    .sort((a, b) => b.balanceCents - a.balanceCents);

  return {
    currency: data?.business?.currency?.code || null,
    accounts,
    totalCents: accounts.reduce((s, a) => s + a.balanceCents, 0),
    // Every account, including the ones sitting at zero, so the UI can say
    // how much of the chart of accounts is actually in use.
    accountCount: conn?.pageInfo?.totalCount ?? accounts.length,
  };
}

// Rolls invoices into the figures the dashboard cash panel wants.
// Invoice-derived only: expenses are account balances to date, not a
// dated series, so they cannot be netted against a period of invoices to
// produce a profit figure. Anything labelled "net" here would be a
